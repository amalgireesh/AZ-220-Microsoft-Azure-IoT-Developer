...
#### Task 1: Explore the application and Connect to IoT Hub
...
2. Minimize the azure portal and open the **IoT Simulator App** on the desktop. Paste the **Connection string** value which you copied earlier in the **Iothub Connectionstring** textbox.

3. Click **Connect** to establish connection with IoT Hub.

4. Explore through the Application for **Registering**, **Activating**, **UnRegistering** of devices, **Sending Telemetry** and so on.

   ![lab 01 iot simulator](media/iotsimulator01.png)
   
   
#### Task 2: Connect device to IoT Hub

1. Click on **Register** which is next to the DeviceId on the IoT Simulator app dialog to register device to the IoThub.

1. Click on **Activate** to simulate changing the device status from disabled to enabled in the IoT Hub Registry.

      > **Note**: Repeat the above steps to register and activate multiple devices.
