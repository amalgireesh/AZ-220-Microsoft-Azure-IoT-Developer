...
#### Task 1: Explore the application //Automation

1. Get the connection string from the Environment Details Tab.
2. Connect the Simulator App with with the IoT Hub Connection String.
3. Explore through the Application for **Registering**, **Activating**, **UnRegister**, **Send Telemetry** and so on.

   ![lab 01 iot simulator](media/iotsimulator.png)
...
* CaveDevice.csproj
* Program.cs //Ref--Device.cs in MCW-Internet-of-Things
...
1. In the Azure Cloud Shell, to monitor the event messages that are being received by your IoT hub, enter the following command:
    ...
    _Be sure to replace both the **{IoTHubName}** and **{Device ID} placeholder with the name of your Azure IoT Hub and with the Device ID like Device01._
